 ...
 */
extern int slurmdb_res_add(void *db_conn, SlurmList res_sl)
{
	List res_list = (List)res_sl;
	if (db_api_uid == -1)
		db_api_uid = getuid();
 ...
 */
extern SlurmList slurmdb_res_get(void *db_conn,
 ...
 * note List needs to be freed with slurm_list_destroy() when called
 */
extern SlurmList slurmdb_res_remove(void *db_conn,
			       slurmdb_res_cond_t *res_cond)
{
	if (db_api_uid == -1)
		db_api_uid = getuid();
